...
export const useDiagramDebug = () => {
  ...
  useEffect(() => {
    // Safety check to make sure SVG is available before accessing it
    if (!svgRef.current) {
      console.log("SVG ref not available yet in useDiagramDebug");
      return;
    }
    ...
    // Try to find all subitems within boxes - Safety check SVG ref again
    setTimeout(() => {
      if (!svgRef.current) {
        console.log("SVG ref not available in delayed check in useDiagramDebug");
        return;
      }
